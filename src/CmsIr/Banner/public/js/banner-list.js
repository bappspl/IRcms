$(function () {
    ...
    if ($('#datatable-banner').length > 0){
                    ...
                    "render": function (data, type, row) {   // o, v contains the object and value for the column
                        if ( type === 'display' ) {
                            return  '<a href="banner/edit/'+data+'" class="btn btn-primary" data-toggle="tooltip" title="Edycja"><i class="fa fa-pencil"></i></a> ' +
                                '<a href="banner/delete/'+data+'" id="'+data+'" class="btn btn-danger" data-toggle="tooltip" title="Usuwanie"><i class="fa fa-trash-o"></i></a>';
                        }
                        return data;
                    },
            ...
            "drawCallback": function(  data ) {
                ...
                setTimeout(function () {
                    ...
                    $('.check-all').on('ifChecked', function (event) {
                        selectedIds = [];
                        $('#datatable-banner tbody tr td .check-row').iCheck('check');
                        //console.log(selectedIds);
                    });
                    ...
                    $('.check-all').on('ifUnchecked', function (event) {
                        selectedIds = [];
                        $('#datatable-banner tbody tr td .check-row').iCheck('uncheck');
                        //console.log(selectedIds);
                    });
                },0);

                //massive action select
                if($('#datatable-banner_length select[name="massive-action"]').length === 0)
                {
                    $('#datatable-banner_length').append($('.massive-action').html());
                }
                            ...
                            if(selectedIds.length > 0)
                            {
                                ...
                                } else // massive change status
                                {
                                    $('#'+modal+' form input').off('click').on('click', function (ev) {
                                        ...
                                        $.ajax({
                                            type: "POST",
                                            url: "/cms-ir/banner/change-status/1",
                                            dataType : 'json',
                                            data: {
                                                modal: true,
                                                id: selectedIds,
        ...
        // delete modal
        $('#datatable-banner tbody').on('click', '.btn-danger', function (e) {
            e.preventDefault();
            ...
            $('#deleteModal').off().on('show.bs.modal', function () {
                ...
                $('#deleteModal form input').off('click').on('click', function (ev) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "/cms-ir/banner/delete/" +entityId,
                        dataType : 'json',
                        data: {
                            modal: true,
                            id: entityId,
