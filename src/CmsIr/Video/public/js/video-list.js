$(function () {
    ...
    if ($('#datatable-video').length > 0){
        selectedIds = [];
        var table = $('#datatable-video').DataTable({
            ...
            "columnDefs": [
                ...
                { "orderData": 2, "targets": [ 3 ] },
                {
                    ...
                    "render": function (data, type, row) {   // o, v contains the object and value for the column
                        if ( type === 'display' ) {
                            return  '<a href="video/edit/'+data+'" class="btn btn-primary" data-toggle="tooltip" title="Edycja"><i class="fa fa-pencil"></i></a> ' +
                                '<a href="video/delete/'+data+'" id="'+data+'" class="btn btn-danger" data-toggle="tooltip" title="Usuwanie"><i class="fa fa-trash-o"></i></a>';
                        }
                        return data;
                    },
            ...
            "drawCallback": function(  data ) {
                ...
                setTimeout(function () {
                    ...
                    $('.check-all').on('ifChecked', function (event) {
                        selectedIds = [];
                        $('#datatable-video tbody tr td .check-row').iCheck('check');
                        //console.log(selectedIds);
                    });
                    ...
                    $('.check-all').on('ifUnchecked', function (event) {
                        selectedIds = [];
                        $('#datatable-video tbody tr td .check-row').iCheck('uncheck');
                        //console.log(selectedIds);
                    });
                },0);

                //massive action select
                if($('#datatable-video_length select[name="massive-action"]').length === 0)
                {
                    $('#datatable-video_length').append($('.massive-action').html());
                }
                            ...
                            if(selectedIds.length > 0)
                            {
                                ...
                                } else // massive change status
                                {
                                    $('#'+modal+' form input').off('click').on('click', function (ev) {
                                        ...
                                        $.ajax({
                                            type: "POST",
                                            url: "/cms-ir/video/change-status/1",
                                            dataType : 'json',
                                            data: {
                                                modal: true,
                                                id: selectedIds,
        ...
        // delete modal
        $('#datatable-video tbody').on('click', '.btn-danger', function (e) {
            e.preventDefault();
            ...
            $('#deleteModal').off().on('show.bs.modal', function () {
                ...
                $('#deleteModal form input').off('click').on('click', function (ev) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "/cms-ir/video/delete/" +entityId,
                        dataType : 'json',
                        data: {
                            modal: true,
                            id: entityId,
        ...
        table.on( 'row-reorder', function ( e, diff, edit ) {
            ...
            $.ajax({
                type: "POST",
                url: "/cms-ir/video/change-position",
                dataType : 'json',
                data: {
                    position: result
                },
