$(function () {
    ...
    if ($('#datatable-gallery').length > 0){
        selectedIds = [];
        var table = $('#datatable-gallery').DataTable({
            ...
            "columnDefs": [
                ...
                { "orderData": 2, "targets": [ 3 ] },
                {
                    ...
                    "render": function (data, type, row) {   // o, v contains the object and value for the column
                        if ( type === 'display' ) {
                            return  '<a href="gallery/edit/'+data+'" class="btn btn-primary" data-toggle="tooltip" title="Edycja"><i class="fa fa-pencil"></i></a> ' +
                                '<a href="gallery/delete/'+data+'" id="'+data+'" class="btn btn-danger" data-toggle="tooltip" title="Usuwanie"><i class="fa fa-trash-o"></i></a>';
                        }
                        return data;
                    },
            ...
            "drawCallback": function(  data ) {
                ...
                setTimeout(function () {
                    ...
                    $('.check-all').on('ifChecked', function (event) {
                        selectedIds = [];
                        $('#datatable-gallery tbody tr td .check-row').iCheck('check');
                        //console.log(selectedIds);
                    });
                    ...
                    $('.check-all').on('ifUnchecked', function (event) {
                        selectedIds = [];
                        $('#datatable-gallery tbody tr td .check-row').iCheck('uncheck');
                        //console.log(selectedIds);
                    });
                },0);

                //massive action select
                if($('#datatable-gallery_length select[name="massive-action"]').length === 0)
                {
                    $('#datatable-gallery_length').append($('.massive-action').html());
                }
                            ...
                            if(selectedIds.length > 0)
                            {
                                ...
                                } else // massive change status
                                {
                                    $('#'+modal+' form input').off('click').on('click', function (ev) {
                                        ...
                                        $.ajax({
                                            type: "POST",
                                            url: "/cms-ir/gallery/change-status/1",
                                            dataType : 'json',
                                            data: {
                                                modal: true,
                                                id: selectedIds,
        ...
        table.on( 'row-reorder', function ( e, diff, edit ) {
            ...
            $.ajax({
                type: "POST",
                url: "/cms-ir/gallery/change-position",
                dataType : 'json',
                data: {
                    position: result
                },
