$(function () {
    ...
    $('#upload-main').uploadifive({
        ...
        'multi'         : true,
        'queueID'          : 'queue-main',
        'uploadScript'     : '/cms-ir/gallery/upload-main',
        'onUploadComplete' : function(file, data) {
            $('#filename-main').val(data);
            if($('#filename-main').val().length > 0) {
                $('.files-main img').remove();
                $('.files-main').append('<div class="deletePhoto_main">  <i class="fa fa-times" data-toggle="tooltip" title="Usuń zdjęcie"></i> <img src="/files/gallery/'+data+'" class="thumb" /> </div>')
            }
            ...
            $('.deletePhoto_main i').on('click', function () {
                ...
                $.ajax({
                    type: "POST",
                    url: "/cms-ir/gallery/delete-photo-main",
                    dataType : 'json',
                    data: {
                        id: id,
                        filePath: fullPathToImage
    ...
    if($('#filename-main').val().length > 0) {
        ...
        $('.files-main').append('<div class="deletePhoto_main">  <i class="fa fa-times" data-toggle="tooltip" title="Usuń zdjęcie"></i> <img src="/files/gallery/'+filename+'" class="thumb" /> </div>')
    }
