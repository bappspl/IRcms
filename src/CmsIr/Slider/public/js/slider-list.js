$(function () {
    ...
    if ($('#datatable-slider').length > 0){
        selectedIds = [];
        var table = $('#datatable-slider').DataTable({
            ...
            "columnDefs": [
                ...
                { "orderData": 3, "targets": [ 4 ] },
                {
                    ...
                    "render": function (data, type, row) {   // o, v contains the object and value for the column
                        if ( type === 'display' ) {
                            return  '<a href="slider/edit/'+data+'" class="btn btn-primary" data-toggle="tooltip" title="Edycja"><i class="fa fa-pencil"></i></a> ' +
            ...
            "drawCallback": function(  data ) {
                ...
                setTimeout(function () {
                    ...
                    $('.check-all').on('ifChecked', function (event) {
                        selectedIds = [];
                        $('#datatable-slider tbody tr td .check-row').iCheck('check');
                        //console.log(selectedIds);
                    });
                    ...
                    $('.check-all').on('ifUnchecked', function (event) {
                        selectedIds = [];
                        $('#datatable-slider tbody tr td .check-row').iCheck('uncheck');
                        //console.log(selectedIds);
                    });
                },0);

                //massive action select
                if($('#datatable-slider_length select[name="massive-action"]').length === 0)
                {
                    $('#datatable-slider_length').append($('.massive-action').html());
                }
                            ...
                            if(selectedIds.length > 0)
                            {
                                ...
                                } else // massive change status
                                {
                                    $('#'+modal+' form input').off('click').on('click', function (ev) {
                                        ...
                                        $.ajax({
                                            type: "POST",
                                            url: "/cms-ir/slider/change-status/1",
                                            dataType : 'json',
                                            data: {
                                                modal: true,
                                                id: selectedIds,
        ...
        // delete modal
        $('#datatable-slider tbody').on('click', '.btn-danger', function (e) {
            e.preventDefault();
            ...
            $('#deleteModal').off().on('show.bs.modal', function () {
                ...
                $('#deleteModal form input').off('click').on('click', function (ev) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "/cms-ir/slider/delete/" +entityId,
                        dataType : 'json',
                        data: {
                            modal: true,
                            id: entityId,
