$(function () {
    ...
    if ($('#datatable-menu').length > 0) {
            ...
            "drawCallback": function (data) {
                ...
                setTimeout(function () {
                    ...
                    $('.check-all').on('ifChecked', function (event) {
                        selectedIds = [];
                        $('#datatable-menu tbody tr td .check-row').iCheck('check');
                        //console.log(selectedIds);
                    });
                    ...
                    $('.check-all').on('ifUnchecked', function (event) {
                        selectedIds = [];
                        $('#datatable-menu tbody tr td .check-row').iCheck('uncheck');
                        //console.log(selectedIds);
                    });
                }, 0);

                //massive action select
                if ($('#datatable-menu_length select[name="massive-action"]').length === 0) {
                    $('#datatable-menu_length').append($('.massive-action').html());
                }
        ...
        // delete modal
        $('#datatable-menu tbody').on('click', '.btn-danger', function (e) {
            e.preventDefault();
            ...
            $('#deleteModal').off().on('show.bs.modal', function () {
                ...
                $('#deleteModal form input').off('click').on('click', function (ev) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "/cms-ir/menu/delete/" +entityId,
                        dataType : 'json',
                        data: {
                            modal: true,
                            id: entityId,
