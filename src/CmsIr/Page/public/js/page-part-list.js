$(function () {
    ...
    if ($('#datatable-page').length > 0){
        ...
        var table = $('#datatable-page').DataTable({
            ...
            "processing": true,
            "serverSide": true,
            "ajaxSource": "/cms-ir/page/part/" + pagePart,
            "serverMethod": "POST",
            "paginate":true,
            "sortable": true,
            "searchable": true,
            ...
            "columnDefs": [
                ...
                },
                {
                    ...
                    "render": function (data, type, row) {   // o, v contains the object and value for the column
                        if(type === 'display') {
                            return '<a href="' + pagePart + '/edit/'+data+'" class="btn btn-primary" data-toggle="tooltip" title="Edycja"><i class="fa fa-pencil"></i></a> ' +
                                '<a href="' + pagePart + '/delete/'+data+'" id="'+data+'" class="btn btn-danger" data-toggle="tooltip" title="Usuwanie"><i class="fa fa-trash-o"></i></a>';
                        }
                        return data;
                    },
                            ...
                            if(selectedIds.length > 0)
                            {
                                ...
                                if(action === 'delete') // massive delete
                                {
                                    $('#'+modal+' form input').off('click').on('click', function (ev) {
                                        ...
                                        $.ajax({
                                            type: "POST",
                                            url: "/cms-ir/page/part/" + pagePart + "/delete/1",
                                            dataType : 'json',
                                            data: {
                                                modal: true,
                                                id: selectedIds,
        ...
        $('#datatable-page tbody').on('click', '.btn-danger', function (e) {
            ...
            $('#deleteModal').off().on('show.bs.modal', function () {
                ...
                $('#deleteModal form input').off('click').on('click', function (ev) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "/cms-ir/page/part/" + pagePart + "/delete/" +entityId,
                        dataType : 'json',
                        data: {
                            modal: true,
                            id: entityId,
        ...
        table.on( 'row-reorder', function ( e, diff, edit ) {
            ...
            $.ajax({
                type: "POST",
                url: "/cms-ir/page/part/" + pagePart +"/change-position",
                dataType : 'json',
                data: {
                    position: result
                },
